...
public class AgentStream {
    ...
    public synchronized SseEmitter open(long user,long id,String token) {
        ...
        dispatch(emitter,c); return emitter;
    }
    ...
    public void refresh() { clients.forEach(this::dispatch); }
    private void dispatch(SseEmitter emitter,Connection c) {
        if(c.closed.get() || !c.sending.compareAndSet(false,true)) return;
        try { senders.execute(()->{try { publish(emitter,c); } finally { c.sending.set(false); }}); }
        catch(RejectedExecutionException e) { c.sending.set(false); }
    }
    private void publish(SseEmitter emitter,Connection c) {
        synchronized(c) {
            ...
            try {
                ...
                long revision=app.revision(c.user,c.id);
                if(c.closed.get()) return;
                if(revision<=c.revision && !(now-c.lastSnapshot>=15000 && app.hasMedia(c.user,c.id))) return;
                var snapshot=app.snapshot(c.user,c.id);
