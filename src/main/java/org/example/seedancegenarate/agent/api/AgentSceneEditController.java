...
public class AgentSceneEditController {
    private final AgentSceneEditApplication edits;
    ...
    @PostMapping("/{id}/scene-edits")
    public Result<AgentViews.Snapshot> edit(@PathVariable long id,@RequestBody AgentSceneEditApplication.Command command) {
        ...
        edits.apply(user,id,command);return Result.success(app.snapshot(user,id));
    }
    ...
    public ResponseEntity<Result<?>> rejected(BusinessException error) {
        ...
        return ResponseEntity.status(code).body(Result.fail(code,code==500?"修改请求暂未确认，请重试原请求":error.getMessage()));
    }
    @ExceptionHandler({HttpMessageNotReadableException.class,MethodArgumentTypeMismatchException.class})
    public ResponseEntity<Result<?>> malformed() {
        return ResponseEntity.badRequest().body(Result.fail(400,"修改请求格式无效"));
    }
    @ExceptionHandler(Exception.class)
    public ResponseEntity<Result<?>> unavailable() {
        return ResponseEntity.internalServerError().body(Result.fail(500,"修改请求暂未确认，请重试原请求"));
    }
}
