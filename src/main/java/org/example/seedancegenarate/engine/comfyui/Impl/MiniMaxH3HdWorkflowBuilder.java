...
/**
 * MiniMax-H3「高清优化版」多参考生视频工作流构建器（model = minimax-h3-hd）。
 * 与优化版的区别是两段采样：一段 6-step 出低分辨率潜空间，再经 3D 潜空间放大器 2 倍
 * （音视频潜空间先分离、放大视频、再拼回）走第二段 ManualSigmas 精修，最终输出 2 倍分辨率的带音频 mp4。
 * 图片 ≤9、视频 ≤2（每段自带音轨）、独立音频 ≤2；运行时注入提示词 / 双段随机种子 / 比例 / megapixels / 时长。
 */
@Component
@RequiredArgsConstructor
public class MiniMaxH3HdWorkflowBuilder implements WorkflowBuilder {

    public static final String MODEL = "minimax-h3-hd";
    private static final String TEMPLATE_PATH = "comfyui/workflows/minimax-h3-hd.json";

    // —— 模板里的固定节点 id ——
    private static final String NODE_PROMPT = "138";        // PrimitiveStringMultiline
    private static final String NODE_SEED_PASS1 = "129";    // RandomNoise（第一段采样）
    private static final String NODE_SEED_PASS2 = "159";    // RandomNoise（放大后精修段）
    ...
    // 模板里的参考图占位节点，build 时整体剥离按实际数量重建
    private static final String[] TEMPLATE_IMAGE_NODES = {"137"};   // LoadImage
...
@Override
    public ModelSpec spec() {
        return new ModelSpec("comfyui", MODEL, "MiniMax-H3多参生视频 高清优化版",
                true, IMAGE_MIN, IMAGE_MAX, RATIOS, DURATION_MIN, DURATION_MAX, List.of(),
                OutputType.VIDEO, MEGAPIXELS, VIDEO_MAX, AUDIO_MAX, true).withImageInputMode(ModelSpec.ImageInputMode.REFERENCE_IMAGE).withResolutions(List.of(
                        new ModelSpec.ResolutionOption("720p", 0.2, true),
                        new ModelSpec.ResolutionOption("1080p", 0.5, true),
                        new ModelSpec.ResolutionOption("2k", 0.9, true)), DEFAULT_MEGAPIXELS);
    }
    ...
    public JsonNode build(GenerateCommand command, ReferenceFiles files) throws Exception {
        ...
        requireInputs(root, NODE_SEED_PASS2).put("noise_seed",
                ThreadLocalRandom.current().nextLong(1L, 1_000_000_000_000_000L));
        // 比例 + 分辨率档位（ResolutionSelector 算宽高，成片再放大 2 倍）
    ...
    private ObjectNode requireInputs(ObjectNode root, String nodeId) {
        ...
        if (node == null || !node.path("inputs").isObject()) {
            throw new IllegalStateException("MiniMax-H3 高清优化版工作流模板缺少节点或 inputs: " + nodeId);
        }
