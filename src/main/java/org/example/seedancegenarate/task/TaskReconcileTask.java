...
public class TaskReconcileTask {
    ...
    /** 自动重试上限（仅 ON_SUCCESS 计费引擎；0 = 只失败不重试） */
     ...
     * PROCESSING → 最后 poll 确认（引擎可能刚好完成）→ 仍无结果则按引擎计费能力决策。
    ...
    /** 决策：ON_SUCCESS 计费引擎免费重试（未超上限入队）；其余超时终止。 */
    private void decideRetry(VideoTask task, VideoEngine engine) {
        int retryCount = task.getRetryCount() == null ? 0 : task.getRetryCount();
        if (engine.billingTiming() == BillingTiming.ON_SUCCESS && retryCount < timeoutRetryMax) {
        ...
        String reason = engine.billingTiming() == BillingTiming.ON_SUCCESS
                ? "任务执行超时，已自动重试 " + retryCount + " 次仍超时，已终止（可手动重试）"
                : "任务执行超时，已终止（提交即计费引擎不自动重试，重新提交可能产生费用）";
